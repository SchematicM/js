...
function createDivObj(user, userDiv) {
...
textDiv.classList.add('text-tag-converter');
let inputDiv = document.createElement('div');
inputDiv.classList.add('text');
inputDiv.setAttribute('contentEditable','true');
let label = document.createElement('label');
label.innerText = 'Input :';
let input = document.createElement('input');
input.setAttribute('placeholder', 'Word for converting to tag ');
inputDiv.append(label,input);
let outputDiv = document.createElement('div');
outputDiv.classList.add('text');
let p = document.createElement('p');
p.innerText = `Output : `;
outputDiv.appendChild(p);
textDiv.append(inputDiv,outputDiv);
document.body.appendChild(textDiv);

input.addEventListener("keydown", (e) => {
    if(e.key === "Tab"){
        p.innerText = `Output : <${input.value}></${input.value}>`;
    }
});
